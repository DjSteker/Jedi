...
Additional Authors (Add your name below): IvanMV 19/03/2018 
1.
License: MIT
...
uifilenameandpath = os.path.join(thisscriptpath, 'EEZYbotArm_Pi_I2C_ui.ui')
...
class DobotGUIApp(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        baseMicrosteppingMultiplier = 1.9
        upperArmMicrosteppingMultiplier = 1.0
        lowerArmMicrosteppingMultiplier = 1.0
        """
    ...
    def move_stepper_motors_using_line_step_sequence(self):
        ...
        WaitTime = 0.0012
        WaitTimeAcel = 0.0012
        PuslsosCuenta = len(self.stepperSequenceToDrawALine)
        PulsosAceleracion = 1
        ...
        for i in range(1, (len(self.stepperSequenceToDrawALine)-1), 6):#note the -1 in the range limit, which accounts for the 'e' end character. Also note that it increments by 6 and starts at index 1 (2nd list item), after 's'
            ...
            #base 
            if ((self.stepperSequenceToDrawALine[i] == 1) and (self.stepperSequenceToDrawALine[i+3] == 1)):
                smbus.SMBus(1).write_byte_data(0x07, 0x03, 0x01)
                time.sleep(WaitTimeAcel +0.0001)
            if ((self.stepperSequenceToDrawALine[i] == 0 )and (self.stepperSequenceToDrawALine[i+3] == 1)):
                smbus.SMBus(1).write_byte_data(0x07, 0x03, 0x02)
                time.sleep(WaitTimeAcel +0.0001)
            #upper arm
            if ((self.stepperSequenceToDrawALine[i+1] == 1) and (self.stepperSequenceToDrawALine[i+4] == 1)):
                smbus.SMBus(1).write_byte_data(0x07, 0x01, 0x01)
            if ((self.stepperSequenceToDrawALine[i+1] == 0) and (self.stepperSequenceToDrawALine[i+4] == 1)):
                smbus.SMBus(1).write_byte_data(0x07, 0x01, 0x02)
            #lower arm 
            if ((self.stepperSequenceToDrawALine[i+2] == 1) and (self.stepperSequenceToDrawALine[i+5] == 1)):
                smbus.SMBus(1).write_byte_data(0x07, 0x02, 0x01)
            if ((self.stepperSequenceToDrawALine[i+2] == 0) and (self.stepperSequenceToDrawALine[i+5] == 1)):
                smbus.SMBus(1).write_byte_data(0x07, 0x02, 0x02)
            
            if(i<PulsosAceleracion):
                tiempo = (WaitTimeAcel - 0.00015)
